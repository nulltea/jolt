use crate::jolt::vm::bytecode::{bytecode_to_val, BytecodePreprocessing, BytecodeShoutProof};
use crate::{
    field::JoltField,
    into_optimal_iter, join_if_rayon, optimal_chunks, optimal_iter, optimal_iter_mut,
    optimal_num_threads, optimal_reduce,
    poly::{
...
impl<F: JoltField, ProofTranscript: Transcript> BytecodeShoutProof<F, ProofTranscript> {
    ...
    pub fn prove(
        ...
        let F: Vec<_> = optimal_reduce!(
            optimal_chunks!(trace, chunk_size)
                .enumerate()
            ...
            |mut running, new| {
                optimal_iter_mut!(running)
                    .zip(into_optimal_iter!(new))
                    .for_each(|(x, y)| *x += y);
                running
            }
        );
        ...
        let rv_claim: F = optimal_iter!(F)
            .zip(optimal_iter!(val))
            .map(|(&ra, &val)| ra * val)
            .sum();
